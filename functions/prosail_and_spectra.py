...
w_cab = w.FloatSlider(value=inv.MEAN_CAB,
                      min=inv.MIN_CAB,
                      max=inv.MAX_CAB,
                      step=1, description=r'Cab ($\mu$g/cm²):', **slide_kwargs)
w_car = w.FloatSlider(value=inv.MEAN_CAR,
                      min=inv.MIN_CAR,
                      max=inv.MAX_CAR,
                      step=1, description=r'Car ($\mu$g/cm²):', **slide_kwargs)
w_ant = w.FloatSlider(value=inv.MEAN_ANT,
                      min=inv.MIN_ANT,
                      max=inv.MAX_ANT,
                      step=1, description=r'Ant ($\mu$g/cm²):', **slide_kwargs)
...
w_tcwv = w.FloatSlider(value=1, min=0, max=10, step=0.1,
                       description='TCWV (g cm⁻²):', **slide_kwargs)
w_aot = w.FloatSlider(value=0.1, min=0, max=1, step=0.01,
                       description='AOT@550nm (-):', **slide_kwargs)
...
w_range = w.FloatRangeSlider(value=RANGE_DICT["LAI"],
                             ...
                             description="Range", readout_format='.1f',
                             **slide_kwargs)
...
def build_random_simulations(n_sim, n_leaf_range, cab_range, car_range, ant_range,
    ...
    params_orig = inv.build_prosail_database(TEST_SIM,
                                             distribution=inv.SALTELLI_DIST)

    soil_spectrum = build_soil_database(params_orig["bs"], soil_files)
    soil_spectrum = np.clip(soil_spectrum, 1e-3, 1)

    rho_canopy_test, params_test = inv.simulate_prosail_lut(params_orig,
                                                      WLS_SIM,
                                                      soil_spectrum,
                                                      skyl=skyl,
                                                      sza=sza,
    ...
    rho_canopy_test = np.maximum(rho_canopy_test, 1e-3)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, f"prosail_{sensor}_model.png"))
    plt.show()
